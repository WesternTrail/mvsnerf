...
class SL1Loss(nn.Module):
    ...
    def forward(self, depth_pred, depth_gt, mask=None):
        if None == mask:
            mask = depth_gt > 0 # 获得到像素值大于0，即不是黑色背景的区域
...
class MVSSystem(LightningModule):
    def __init__(self, args):
        ...
        self.args = args
        self.args.feat_dim = 8+3*4 # 为什么是8+3*4?
        self.idx = 0
    ...
    def decode_batch(self, batch, idx=list(torch.arange(4))):
        ...
        data_mvs = sub_selete_data(batch, device, idx, filtKey=[]) # 就是把数据都搬到GPU上去
    ...
    def training_step(self, batch, batch_nb): # 重写lighting中train每一个batch数据
        if 'scan' in batch.keys():
            batch.pop('scan')
        log, loss = {},0
        data_mvs, pose_ref = self.decode_batch(batch) # 获得
        imgs, proj_mats = data_mvs['images'], data_mvs['proj_mats'] # 获得一个batch的图片，和从参考图像到源图像的[R|T]
        near_fars, depths_h = data_mvs['near_fars'], data_mvs['depths_h']


        volume_feature, img_feat, depth_values = self.MVSNet(imgs[:, :3], proj_mats[:, :3], near_fars[0,0],pad=args.pad) # 不把最后一个target图像加进去
    ...
    def validation_step(self, batch, batch_nb):
        ...
        with torch.no_grad():
            ...
            imgs = self.unpreprocess(imgs)
            rgbs, depth_preds = [],[]
            for chunk_idx in range(H*W//args.chunk + int(H*W%args.chunk>0)): # 一次训练1024条光线

                # rays_pts (1024,128,3) = (光线数量，光线上采样点个数，3)
...
if __name__ == '__main__':
    ...
    args = config_parser()
    system = MVSSystem(args) # 初始化mvsnerf网络模型
    ...
    trainer = Trainer(max_epochs=args.num_epochs,
                      ...
                      check_val_every_n_epoch = max(system.args.num_epochs//system.args.N_vis,1), # 每1个epoch检查一次
                      benchmark=True,
                      precision=16 if args.use_amp else 32, # 使用混合精度？
                      amp_level='O1')
