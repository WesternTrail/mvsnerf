...
class MVSNeRF(nn.Module):
    def __init__(self, D=8, W=256, input_ch_pts=3, input_ch_views=3, input_ch_feat=8, skips=[4], net_type='v2'):
        ...
        self.in_ch_pts, self.in_ch_views,self.in_ch_feat = input_ch_pts, input_ch_views, input_ch_feat # 63,3,20
...
def create_nerf_mvs(args, pts_embedder=True, use_mvs=False, dir_embedder=True):
    ...
    if pts_embedder: # 对坐标位置进行embeding
        embed_fn, input_ch = get_embedder(args.multires, args.i_embed, input_dims=args.pts_dim) # input_ch = 20x3 + 3= 63
    else:
        embed_fn, input_ch = None, args.pts_dim

    embeddirs_fn = None
    if dir_embedder: # 不对view_dir做embeding
    ...
    model = MVSNeRF(D=args.netdepth, W=args.netwidth,
                 ...
                 input_ch_views=input_ch_views, input_ch_feat=args.feat_dim, net_type=args.net_type).to(device) # 跟传统的NeRF差不多。但是有一些变化

    grad_vars = []
    grad_vars += list(model.parameters())

    model_fine = None
    if args.N_importance > 0: # 没有定义细的网络，
    ...
    EncodingNet = None
    if use_mvs:
        EncodingNet = MVSNet().to(device) # 创建feature encoder和cost reg
    ...
    render_kwargs_train = {
        ...
        'network_fine': model_fine, # 不采用细网络
        'N_samples': args.N_samples, # 粗网络采样128个点
        'network_fn': model, # NeRF网络
        'network_mvs': EncodingNet, # MVS网络结构，包括特征提取部分，和cost volume正则化部分
...
class CostRegNet(nn.Module):
    ...
    def forward(self, x): # (B,C,D,W,H) = (1,32+9,128,176,208)
        conv0 = self.conv0(x) # 开始下采样 -> (1,8,128,176,208)
        conv2 = self.conv2(self.conv1(conv0)) # -> (1,16,64,88,104)
        conv4 = self.conv4(self.conv3(conv2)) # -> (1,32,32,44,52)

        x = self.conv6(self.conv5(conv4)) # -> (1,64,16,22,26)
        x = conv4 + self.conv7(x) # 开始上采样 -> (1,32,32,44,52)
        del conv4 # 删除不需要用到的空间
        x = conv2 + self.conv9(x) # -> (1,16,64,88,104)
        del conv2
        x = conv0 + self.conv11(x) # -> (1,8,128,176,208)
        del conv0
        # x = self.conv12(x)
        return x
...
class MVSNet(nn.Module):
    def __init__(self,
        ...
        self.n_depths = [128,32,8] # 降采样的channel维度？
        self.G = num_groups  # number of groups in groupwise correlation
        self.feature = FeatureNet() # 定义特征提取网络

        self.N_importance = 0
        self.chunk = 1024

        self.cost_reg_2 = CostRegNet(32+9, norm_act) # 3D unet网络
    ...
    def build_volume_costvar_img(self, imgs, feats, proj_mats, depth_values, pad=0):
        ...
        B, V, C, H, W = feats.shape
        D = depth_values.shape[1] # 按照粗采样点的个数选取
        ref_feats, src_feats = feats[:, 0], feats[:, 1:]
        src_feats = src_feats.permute(1, 0, 2, 3, 4)  # (V-1, B, C, h, w)
        proj_mats = proj_mats[:, 1:]
        proj_mats = proj_mats.permute(1, 0, 2, 3)  # (V-1, B, 3, 4)

        if pad > 0: # 对H,W上下左右都填充2xpad的像素
            ref_feats = F.pad(ref_feats, (pad, pad, pad, pad), "constant", 0)

        img_feat = torch.empty((B, 9 + 32, D, *ref_feats.shape[-2:]), device=feats.device, dtype=torch.float) # 创建一个没有初始化的特征，加入深度维度D
        imgs = F.interpolate(imgs.view(B * V, *imgs.shape[2:]), (H, W), mode='bilinear', align_corners=False).view(B, V,-1,H,W).permute(1, 0, 2, 3, 4) # 将原始图像进行下采样到特征图相同的大小
        img_feat[:, :3, :, pad:H + pad, pad:W + pad] = imgs[0].unsqueeze(2).expand(-1, -1, D, -1, -1) # 9的前3维是参考图像的color通道

        ref_volume = ref_feats.unsqueeze(2).repeat(1, 1, D, 1, 1)  # 将参考图像的特征图，扩充一个维度D，复制D份。得到(B, C, D, h, w)
        ...
        for i, (src_img, src_feat, proj_mat) in enumerate(zip(imgs[1:], src_feats, proj_mats)):
            warped_volume, grid = homo_warp(src_feat, proj_mat, depth_values, pad=pad) # grid是参考图像的每个深度像素平面的位置映射到参考图像的位置
            img_feat[:, (i + 1) * 3:(i + 2) * 3], _ = homo_warp(src_img, proj_mat, depth_values, src_grid=grid, pad=pad) # 这是把源图想像素也wrap上去？
        ...
        img_feat[:, -32:] = volume_sq_sum * count - (volume_sum * count) ** 2 # 平方的和减去和的平方？？
        del volume_sq_sum, volume_sum, count
    ...
    def forward(self, imgs, proj_mats, near_far, pad=0,  return_color=False, lindisp=False):
        ...
        imgs = imgs.reshape(B * V, 3, H, W) # 把一个batch中的所有图片合并在一起
        feats = self.feature(imgs)  # (B*V,3,H,W) -> (B*V, 8, H, W) -> (B*V, 16, H//2, W//2) -> (B*V, 32, H//4, W//4)

        imgs = imgs.view(B, V, 3, H, W) # 重新拆开考虑
        ...
        D = 128
        t_vals = torch.linspace(0., 1., steps=D, device=imgs.device, dtype=imgs.dtype)  # (B, D),把0-1的单位长度分为128分
        near, far = near_far  # assume batch size==1
        if not lindisp: # 均匀采样的方式
            depth_values = near * (1.-t_vals) + far * (t_vals)
        else:
            depth_values = 1. / (1. / near * (1. - t_vals) + 1. / far * (t_vals))

        depth_values = depth_values.unsqueeze(0) # 添加一个维度
        # volume_feat, in_masks = self.build_volume_costvar(feats_l, proj_mats, depth_values, pad=pad)
        volume_feat, in_masks = self.build_volume_costvar_img(imgs, feats_l, proj_mats, depth_values, pad=pad) # 构建cost volume，并且进行方差计算
        if return_color:
            feats_l = torch.cat((volume_feat[:,:V*3].view(B, V, 3, *volume_feat.shape[2:]),in_masks.unsqueeze(2)),dim=2)


        volume_feat = self.cost_reg_2(volume_feat)  # (B, 8, D, h, w)
