...
class MVSDatasetDTU(Dataset):
    def __init__(self, root_dir, split, n_views=3, levels=1, img_wh=None, downSample=1.0, max_len=-1):
        ...
        self.build_metas() # 读取trainlist
        self.n_views = n_views
        self.levels = levels  # FPN levels
        self.build_proj_mats() # 创建每50张视角的投影矩阵
    ...
    def build_metas(self):
        ...
        light_idxs = [3] if 'train' != self.split else range(7) # 光照条件0-6都用于训练，测试的时候只用3号
    ...
    def __getitem__(self, idx):
        ...
        if self.split=='train':
            ids = torch.randperm(5)[:3] # 随机地选了3个源视角
            view_ids = [src_views[i] for i in ids] + [target_view]
        else:
            view_ids = [src_views[i] for i in range(3)] + [target_view] # 选择前3张源视角?
        ...
        for i, vid in enumerate(view_ids):
            ...
            depth_filename = os.path.join(self.root_dir,
                                          f'Depths/{scan}/depth_map_{vid:04d}.pfm') # 注意这里并没使用到

            img = Image.open(img_filename)
            img_wh = np.round(np.array(img.size) * self.downSample).astype('int')
            img = img.resize(img_wh, Image.BILINEAR) # 没变化
            img = self.transform(img) # 均值归一化？
            imgs += [img]
            ...
            if i == 0:  # reference view,源视角的第一个做为参考视角
                ref_proj_inv = np.linalg.inv(proj_mat_ls) # 参考视角的投影矩阵的逆矩阵
                proj_mats += [np.eye(4)] # 表示从参考视角到其他视角的[R|T]
            else:
        ...
        sample['images'] = imgs  # (V, H, W, 3),1个参考视角，2个源视角，1个目标视角
        ...
        sample['light_id'] = np.array(light_idx) # 光照的索引
        sample['affine_mat'] = affine_mat # 放射变化矩阵
